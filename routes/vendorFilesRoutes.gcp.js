 ...
 * Replace vendorFilesRoutes.js with this file after migration
 */
...
router.post('/upload-requests', isAuthenticated, isVendorFiles, async (req, res) => {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const uploadUrl = await getSignedUrl(key, {
        action: 'write',
        expiresIn: 60 * 15,
  ...
  try {
    ...
    await Promise.all(
      validKeys.map(async key => {
        const signedUrl = await getSignedUrl(key, { expiresIn: SIGNED_URL_TTL });
