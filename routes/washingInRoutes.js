...
// GET /washingin
router.get('/', isAuthenticated, isWashingInMaster, (req, res) => {
  res.render('washingInEvents', { user: req.session.user });
});

router.get('/legacy', isAuthenticated, isWashingInMaster, async (req, res) => {
  try {
...
router.get('/event/history', isAuthenticated, isWashingInMaster, async (req, res) => {
  try {
    const userId = req.session.user.id;
    ...
    const [sizes] = await pool.query(
      `SELECT event_id, size_label, pieces FROM washing_in_event_sizes WHERE event_id IN (?)`,
      [eventIds]
    );
    const sizeMap = {};
    ...
    console.error('[ERROR] GET /washingin/event/history =>', err);
    res.status(500).json({ error: err.message });
  }
});

router.get('/event/lot-journey/:cuttingLotId', isAuthenticated, isWashingInMaster, async (req, res) => {
  try {
    ...
    console.error('[ERROR] GET /washingin/event/lot-journey =>', err);
    res.status(500).json({ error: err.message });
  }
});

router.get('/event/export', isAuthenticated, isWashingInMaster, async (req, res) => {
  try {
    const userId = req.session.user.id;
    ...
    if (events.length) {
      const eventIds = events.map(e => e.id);
      const [sizes] = await pool.query(
        `SELECT event_id, size_label, pieces FROM washing_in_event_sizes
    ...
    wb.created = new Date();
    const ws = wb.addWorksheet('Washing In Events');
    ws.columns = [
    ...
    const fname = `washing_in_${username}_${days}d_${new Date().toISOString().slice(0, 10)}.xlsx`;
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${fname}"`);
    await wb.xlsx.write(res);
    res.end();
  } catch (err) {
    console.error('[ERROR] GET /washingin/event/export =>', err);
    res.status(500).json({ error: err.message });
  }
});

router.get('/event/payments', isAuthenticated, isWashingInMaster, async (req, res) => {
  try {
    const userId = req.session.user.id;
    const days = Math.max(1, Math.min(365, parseInt(req.query.days, 10) || 30));
    ...
    const [[summary]] = await pool.query(
      `SELECT
         ...
         COUNT(*)                                                                     AS total_rows
       FROM stage_payments
       WHERE user_id = ? AND stage = 'washing_in'
         AND created_at >= (NOW() - INTERVAL ? DAY)`,
      [userId, days]
    );
    ...
    console.error('[ERROR] GET /washingin/event/payments =>', err);
    res.status(500).json({ error: err.message });
  }
});

router.get('/event/payments/export', isAuthenticated, isWashingInMaster, async (req, res) => {
  try {
    const userId = req.session.user.id;
    const username = req.session.user.username || 'operator';
    const days = Math.max(1, Math.min(365, parseInt(req.query.days, 10) || 30));
    ...
    wb.creator = 'Kotty Track';
    wb.created = new Date();
    const ws = wb.addWorksheet('Washing In Payments');
    ws.columns = [
    ...
    const fname = `washing_in_payments_${username}_${days}d_${new Date().toISOString().slice(0, 10)}.xlsx`;
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${fname}"`);
    await wb.xlsx.write(res);
    res.end();
  } catch (err) {
    console.error('[ERROR] GET /washingin/event/payments/export =>', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/event/lot-state/:cuttingLotId', isAuthenticated, isWashingInMaster, async (req, res) => {
  try {
    ...
    const openApprovals  = await stageEvents.getOpenApprovals(pool, STAGE_WI, lotId, userId);
